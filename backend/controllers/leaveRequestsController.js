...
exports.create = async (req, res) => {
  try {
    const { course_id, start_date, end_date, reason, leave_type, is_half_day, half_day_type } = req.body;
    ...
    const [result] = await db.query(
      `INSERT INTO leave_requests 
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'PENDING', NOW())`,
      [student_id, course_id || null, leave_type || 'CASUAL', start_date, end_date, reason, is_half_day || 0, half_day_type || null]
    );
...
exports.getAll = async (req, res) => {
  try {
    ...
    if (userType === 'student') {
      ...
      // Faculty sees leaves for courses they teach
...
exports.updateStatus = async (req, res) => {
  try {
    ...
    const updateData = status === 'APPROVED'
      ? { status, approved_by: userId, approved_at: new Date(), rejected_at: null, rejection_reason: null }
      : { status, approved_by: null, approved_at: null, rejected_at: new Date(), rejection_reason: rejection_reason || null };

    await db.query(
      `UPDATE leave_requests 
       SET status = ?, approved_by = ?, approved_at = ?, rejected_at = ?, rejection_reason = ?, updated_at = NOW() 
       WHERE id = ?`,
      [updateData.status, updateData.approved_by, updateData.approved_at, updateData.rejected_at, updateData.rejection_reason, id]
    );
    ...
    if (status === 'APPROVED' && leave[0].course_id) {
      await db.query(
        ...
        [leave[0].student_id, leave[0].course_id, leave[0].start_date, leave[0].end_date]
      );
    }
